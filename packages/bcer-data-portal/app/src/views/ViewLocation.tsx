...
export default function ViewLocations() {
  ...
  const MOHuser = config.permissions.MANAGE_USERS; //if the user is logged in as MOH user, only MOH admin user will be able to see the "Permanently Delete" button
...
function LocationsContent_MOHuser() {
...
function LocationsContent_HAuser() {
  const classes = useStyles();
  const history = useHistory();
  const { values, setFieldValue }: {values: any, setFieldValue: Function} = useFormikContext();
  const [appGlobal, setAppGlobalContext] = useContext(AppGlobalContext);
